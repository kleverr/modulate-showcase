...
(function () {
  ...
  const optRedactLanguage       = document.getElementById('redact-opt-language');
  ...
  (function populateLanguageOptions() {
    ...
    const sorted = LANGS.slice().sort((a, b) => a[1].localeCompare(b[1]));
    [optLanguage, document.getElementById('redact-opt-language')].forEach(select => {
      if (!select) return;
  ...
  function getRedactionOptions() {
    ...
    // Optional ISO 639-1 hint; omitted = the API auto-detects per utterance.
    if (optRedactLanguage && optRedactLanguage.value) opts.language = optRedactLanguage.value;
    return opts;
  }
  ...
  async function startRedactionBatch(file) {
    ...
    try {
      ...
      currentMeta = {
        ...
        fileType: file.type || file.name.split('.').pop().toUpperCase(),
        processingMs,
        languageHint: opts.language || null,
      };
    ...
    if (currentMode === 'music') {
      ...
      groups = [
        { group: 'Redaction', rows: [
          ...
          ['Language hint', m.languageHint || 'Auto-detect'],
        ]},
        { group: 'Audio', rows: [
          ['File Name', currentData.filename || 'N/A'],
          ['File Size', m.fileSize ? formatBytes(m.fileSize) : 'N/A'],
  ...
  // differs from the standard single `upload_file`.
